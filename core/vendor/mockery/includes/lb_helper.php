...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = 'D2A958AB';
		$this->api_url = 'https://megasoft.biz/licensebox/';
		$this->api_key = 'C1A3398CB1F1D3856427';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["b067a815defd50a"])){
				$_SESSION["b067a815defd50a"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["b067a815defd50a"])){
				$get_data = $this->call_api(
					'POST',
